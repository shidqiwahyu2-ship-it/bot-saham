...
# --- AMBIL DATA MENGGUNAKAN GOAPI.IO (DENGAN HEADER AUTH) ---
    ...
    def _fetch():
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as response:
                ...
                vwap_val = round((high_p + low_p + cp) / 3)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    pesan = (
        ...
        "• `/cek KJEN` - Analisa 1 saham spesifik\n"
...
async def wajib_pantau(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tasks = [fetch_stock_async(ticker) for ticker in set(WATCHLIST)]
    results = await asyncio.gather(*tasks)

    under_1000_list = [d for d in results if d and d["price"] < 1000]

    if not under_1000_list:
        await msg.edit_text("❌ Gagal mengambil data pasar dari GoAPI.")
        return
...
async def cek_saham(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Format salah! Gunakan: `/cek KJEN`", parse_mode="Markdown")
        return
...
async def pantau_saham(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Format salah! Gunakan: `/pantau KJEN`", parse_mode="Markdown")
        return
